...
function App() {
  ...
  const [selectedFabricImage, setSelectedFabricImage] =
  ...
  useEffect(() => {
    ...
    const drawClick$ = fromEvent(drawBtn, "click").pipe(
      map(() => new SwitchMode(Mode.Draw)),
    )
    const panClick$ = fromEvent(panBtn, "click").pipe(
      map(() => new SwitchMode(Mode.Pan)),
    )
    ...
    const upArrowPress$ = key$("keydown", "ArrowUp").pipe(
      map(() => new NextImage()),
    )
    const downArrowPress$ = key$("keydown", "ArrowDown").pipe(
      map(() => new PrevImage()),
    )

    const canvasMouseMove$ = createFabricEventObservable(
      fabricCanvasRef.current,
      "mouse:move",
    ) as Observable<FabricMouseEvent>
    const canvasMouseDown$ = createFabricEventObservable(
      fabricCanvasRef.current,
      "mouse:down",
    ) as Observable<FabricMouseEvent>
    const canvasMouseUp$ = createFabricEventObservable(
      fabricCanvasRef.current,
      "mouse:up",
    ) as Observable<FabricMouseEvent>

    const canvasMouseMoveAction$ = canvasMouseMove$.pipe(
      map(
        (event) =>
          new MouseMove(
            event.e.offsetX,
    ...
    const source$: Observable<State> = action$.pipe(
  ...
  return (
    <div className="h-screen w-screen overflow-hidden">
      ...
      {selectedLabelIndex === null && showLoadPopup === false ? (
        <SelectLabelPopup />
      ) : (
        <></>
      )}
